 ...
 * Parámetros del backend: fecha_inicio, fecha_fin, estado_novedad_id, prioridad_actual, sector_id, tipo_novedad_id, search, page, limit
 */
...
export async function cambiarEstadoNovedad(id, estado_id, observaciones) {
  const res = await api.patch(`/novedades/${id}/estado`, { estado_id, observaciones })
  return res?.data
}
...
export async function listSubtiposNovedad(tipoId) {
  const url = tipoId ? `/subtipos-novedad?tipo_novedad_id=${tipoId}` : '/subtipos-novedad'
...
export async function listUbigeos(search = '') {
  const url = search ? `/catalogos/ubigeo?search=${search}&limit=50` : '/catalogos/ubigeo?limit=50'
  const res = await api.get(url)
  return res?.data?.data || res?.data || []
}
...
export async function listCuadrantes(sectorId) {
  const url = sectorId ? `/cuadrantes?sector_id=${sectorId}&limit=100` : '/cuadrantes?limit=100'
  const res = await api.get(url)
  // Backend responde: { success: true, data: { cuadrantes: [...], pagination: {...} } }
  return res?.data?.data?.cuadrantes || res?.data?.cuadrantes || res?.data?.data || res?.data || []
}
