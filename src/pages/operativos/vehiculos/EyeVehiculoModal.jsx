/**
 * File: src/pages/operativos/vehiculos/EyeVehiculoModal.jsx
 * @version 1.0.0
 * @description Modal READ ONLY para consultar datos completos de operativo vehículo
 * @module src/pages/operativos/vehiculos/EyeVehiculoModal.jsx
 */

import React, { useState, useEffect, useCallback } from "react";
import { X, Eye, Calendar, Clock, MapPin, Car, User, AlertTriangle, CheckCircle } from "lucide-react";
...
const EyeVehiculoModal = ({ 
  ...
  useEffect(() => {
    ...
    const fetchOperativoData = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await operativosNovedadesService.getEyeVehiculoNovedad(
          turnoId, 
          vehiculoId, 
          cuadranteId, 
          operativoId
    ...
    fetchOperativoData();
  }, [isOpen, operativoId, turnoId, vehiculoId, cuadranteId]);

  // Manejo de tecla ESC - Solución directa con evento en el modal
  ...
  const formatVehiculo = (vehiculo) => {
    ...
    return `${vehiculo.codigo_vehiculo} ${vehiculo.placa} ${vehiculo.marca} ${vehiculo.modelo_vehiculo}`;
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900 dark:text-white">
                  Consulta de Operativo Vehículo
                </h2>
              ...
              <div className="bg-gray-50 dark:bg-slate-800 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-gray-100">{data.cuadranteOperativo?.operativoVehiculo?.turno?.turno || 'N/A'}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Fecha Turno:</span>
                    <p className="font-medium text-gray-900 dark:text-gray-100">{data.cuadranteOperativo?.operativoVehiculo?.turno?.fecha || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-gray-100">{data.cuadranteOperativo?.cuadrante?.cuadrante_code || 'N/A'}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Nombre Cuadrante:</span>
                    <p className="font-medium text-gray-900 dark:text-gray-100">{data.cuadranteOperativo?.cuadrante?.nombre || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-gray-100">{formatVehiculo(data.cuadranteOperativo?.operativoVehiculo?.vehiculo)}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Conductor:</span>
                    <p className="font-medium text-gray-900 dark:text-gray-100">{formatPersonal(data.cuadranteOperativo?.operativoVehiculo?.conductor)}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div>
                    ...
                    <p className="font-medium text-gray-900 dark:text-gray-100">{formatUsuario(data.creadorOperativosVehiculosNovedades)}</p>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600 dark:text-gray-400">Actualizado por:</span>
                    <p className="font-medium text-gray-900 dark:text-gray-100">{formatUsuario(data.actualizadorOperativosVehiculosNovedades)}</p>
                  </div>
                  <div>
