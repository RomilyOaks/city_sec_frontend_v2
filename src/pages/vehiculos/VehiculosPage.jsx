...
import { Car, ChevronLeft, ChevronRight, Eye, Pencil, Plus, RefreshCw, Search, Trash2, RotateCcw, X } from 'lucide-react'

import { listVehiculos, deleteVehiculo, restoreVehiculo, createVehiculo, updateVehiculo } from '../../services/vehiculosService.js'
import { listTiposVehiculo, listUnidades } from '../../services/catalogosService.js'
...
// Formatear número con separador de miles (coma para Perú)
...
export default function VehiculosPage() {
  ...
  useEffect(() => { showCreateModalRef.current = showCreateModal }, [showCreateModal])
  useEffect(() => { editingVehiculoRef.current = editingVehiculo }, [editingVehiculo])
  ...
  const handleCreate = async () => {
    ...
    try {
      const payload = {
        ...
        anio_vehiculo: formData.anio_vehiculo ? Number(formData.anio_vehiculo) : undefined,
        ...
        kilometraje_inicial: formData.kilometraje_inicial ? Number(formData.kilometraje_inicial) : undefined,
        kilometraje_actual: formData.kilometraje_actual ? Number(formData.kilometraje_actual) : undefined,
        capacidad_combustible: formData.capacidad_combustible ? Number(formData.capacidad_combustible) : undefined,
        soat: formData.soat?.trim() || undefined,
        fec_soat: formData.fec_soat || undefined,
        fec_manten: formData.fec_manten || undefined,
        conductor_asignado_id: formData.conductor_asignado_id ? Number(formData.conductor_asignado_id) : undefined,
  ...
  const estadoColor = (estado) => {
    switch (estado) {
      case 'DISPONIBLE': return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300'
      case 'EN_SERVICIO': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
      case 'MANTENIMIENTO': return 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300'
      case 'REPARACION': return 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300'
      case 'FUERA_DE_SERVICIO': return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'
    ...
    <div className="space-y-4">
      ...
      {activeTab === 0 && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Tipo de Vehículo *</label>
            <select
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Placa *</label>
            <input
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Nombre / Alias</label>
            <input
              ...
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
            <p className="mt-1 text-xs text-slate-500">Nombre corto o alias para identificar el vehículo fácilmente</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Marca</label>
            <input
              value={formData.marca}
              onChange={(e) => setFormData({ ...formData, marca: capitalize(e.target.value) })}
              placeholder="Ej: Toyota"
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Modelo</label>
            <input
              value={formData.modelo_vehiculo}
              onChange={(e) => setFormData({ ...formData, modelo_vehiculo: capitalize(e.target.value) })}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Color</label>
            <input
              value={formData.color_vehiculo}
              onChange={(e) => setFormData({ ...formData, color_vehiculo: capitalize(e.target.value) })}
          ...
          <div className="col-span-2">
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, unidad_oficina_id: e.target.value })}
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            >
              <option value="">— Seleccione —</option>
              {unidades.map((u) => (
          ...
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Estado Operativo</label>
            <select
          ...
          <div className="col-span-2 mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p className="text-sm text-blue-700 dark:text-blue-300 flex items-center gap-2">
              ...
              <span>
                <strong>Tip:</strong> Puede agregar más información usando las pestañas 
                <button type="button" onClick={() => setActiveTab(1)} className="font-semibold underline hover:text-blue-900 dark:hover:text-blue-100">Datos Técnicos</button> y 
                <button type="button" onClick={() => setActiveTab(2)} className="font-semibold underline hover:text-blue-900 dark:hover:text-blue-100">Documentos</button>.
              </span>
            </p>
          </div>
        </div>
      ...
      {activeTab === 1 && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Número de Motor</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Número de Chasis</label>
            <input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Kilometraje Inicial</label>
            <input
              type="text"
              inputMode="numeric"
              value={formatNumber(formData.kilometraje_inicial)}
              onChange={(e) => setFormData({ ...formData, kilometraje_inicial: parseFormattedNumber(e.target.value) })}
              placeholder="Ej: 10,500"
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Kilometraje Actual</label>
            <input
              type="text"
              inputMode="numeric"
              value={formatNumber(formData.kilometraje_actual)}
              onChange={(e) => setFormData({ ...formData, kilometraje_actual: parseFormattedNumber(e.target.value) })}
              placeholder="Ej: 25,000"
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Capacidad Combustible (galones)</label>
            <input
              type="number"
              step="0.01"
              value={formData.capacidad_combustible}
              onChange={(e) => setFormData({ ...formData, capacidad_combustible: e.target.value })}
              min="0"
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
      ...
      {activeTab === 2 && (
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Número SOAT</label>
            <input
              value={formData.soat}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Vencimiento SOAT</label>
            <input
              type="date"
              value={formData.fec_soat}
              onChange={(e) => setFormData({ ...formData, fec_soat: e.target.value })}
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Próximo Mantenimiento</label>
            <input
              type="date"
              value={formData.fec_manten}
              onChange={(e) => setFormData({ ...formData, fec_manten: e.target.value })}
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            />
          </div>
          <div className="col-span-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Conductor Asignado</label>
            <select
              value={formData.conductor_asignado_id}
              onChange={(e) => setFormData({ ...formData, conductor_asignado_id: e.target.value })}
              className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
            >
              <option value="">— Sin asignar —</option>
              {conductores.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.nombres} {c.apellido_paterno} {c.apellido_materno} - {c.licencia || 'Sin licencia'}
                </option>
              ))}
            </select>
            <p className="mt-1 text-xs text-slate-500">Solo personal con licencia de conducir</p>
          </div>
          <div className="col-span-2">
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Gestión de la flota vehicular</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 p-4">
        <div className="grid grid-cols-1 md:grid-cols-[1fr_200px_auto] gap-3">
          ...
          >
            ...
            {ESTADO_OPERATIVO_OPTIONS.map((e) => <option key={e.value} value={e.value}>{e.label}</option>)}
          </select>
          <button
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Placa</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Marca / Modelo</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Unidad</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                vehiculos.map((v) => (
                  <tr key={v.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${v.deleted_at ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${estadoColor(v.estado_operativo)}`}>
                        {ESTADO_OPERATIVO_OPTIONS.find(e => e.value === v.estado_operativo)?.label || v.estado_operativo}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Nuevo Vehículo</h2>
      ...
      {editingVehiculo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Editar Vehículo: {editingVehiculo.placa}</h2>
              <button onClick={() => { setEditingVehiculo(null); resetForm() }} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
                <X size={18} />
              </button>
            </div>
            <div className="p-4">
            ...
            <div className="flex items-center justify-between p-4 border-t border-slate-200 dark:border-slate-700">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => { setEditingVehiculo(null); resetForm() }}
                  className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800"
                >
                  Cancelar
                </button>
      ...
      {viewingVehiculo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Detalle del Vehículo</h2>
              <button onClick={() => setViewingVehiculo(null)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">
                <X size={18} />
              </button>
            </div>
            <div className="p-4 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-slate-900 dark:text-slate-50">{viewingVehiculo.placa}</h3>
                  <p className="text-slate-500">{viewingVehiculo.marca} {viewingVehiculo.modelo_vehiculo}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  ...
                  <p className="font-medium text-slate-900 dark:text-slate-50">{getTipoNombre(viewingVehiculo.tipo_id)}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Unidad</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{getUnidadNombre(viewingVehiculo.unidad_oficina_id)}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Año</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{viewingVehiculo.anio_vehiculo || '—'}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Color</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{viewingVehiculo.color_vehiculo || '—'}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Estado</p>
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${estadoColor(viewingVehiculo.estado_operativo)}`}>
                    {ESTADO_OPERATIVO_OPTIONS.find(e => e.value === viewingVehiculo.estado_operativo)?.label || viewingVehiculo.estado_operativo}
                  </span>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Kilometraje</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{viewingVehiculo.kilometraje_actual?.toLocaleString() || '0'} km</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">SOAT</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{viewingVehiculo.soat || '—'}</p>
                </div>
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400">Venc. SOAT</p>
                  <p className="font-medium text-slate-900 dark:text-slate-50">{viewingVehiculo.fec_soat || '—'}</p>
                </div>
              </div>

              {viewingVehiculo.observaciones && (
                <div className="p-3 bg-slate-50 dark:bg-slate-800/50 rounded-lg">
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Observaciones</p>
                  <p className="text-slate-900 dark:text-slate-50">{viewingVehiculo.observaciones}</p>
                </div>
              )}
            </div>
