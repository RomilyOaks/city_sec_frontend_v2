/**
 * File: c:\\Project\\city_sec_frontend_v2\\src\\pages\\admin\\AdminUsuariosPage.jsx
 * @version 2.0.0
 * @description Página para administración de usuarios: creación, edición, filtros, paginación y control de estado.
 * Incluye validación del formulario mediante Zod y helpers para verificación de unicidad.
 *
 * Documentación educativa: se añadieron JSDoc y comentarios explicativos en funciones clave manteniendo la lógica.
 *
 * @module src/pages/admin/AdminUsuariosPage.jsx
 */
 ...
 * @component
 * @category Pages
 * @description Gestión completa de usuarios: creación con validación Zod, filtros y acciones (activar, desactivar, restaurar).
 *
 * Nota: Se agregaron comentarios y JSDoc para facilitar aprendizaje y mantenimiento sin cambiar la lógica.
 *
 * @returns {JSX.Element}
 */
...
export default function AdminUsuariosPage() {
  ...
  /**
   * capitalize
   * Convierte una cadena para que la primera letra de cada palabra quede en mayúscula.
   *
   * @param {string} str
   * @returns {string}
   */
  ...
  /**
   * checkUserExists
   * - Verifica si un username o email ya existe (incluye registros eliminados) para evitar duplicados.
   *
  ...
  const checkUserExists = async (username, email) => {
    ...
    try {
      ...
      if (existe) {
        setUserExistsError(
          "El username o email ya fueron utilizados anteriormente. Por favor use otros datos."
        );
      } else {
        setUserExistsError("");
      }
  ...
  /**
   * fetchUsers
   * - Consulta paginada de usuarios según filtros (search, rol, estado o deleted)
   * - Actualiza `users` y `usersPagination`.
   *
   * @param {Object} options
   * @param {number} [options.nextPage]
   * @param {string} [options.nextSearch]
   * @param {string} [options.nextRole]
   * @param {string} [options.nextDeleted]
   * @returns {Promise<void>}
   */
  const fetchUsers = async ({
    ...
    nextSearch = search,
    nextRole = filterRole,
    nextDeleted = filterDeleted,
  } = {}) => {
    try {
      setUsersLoading(true);
      // Determinar si es filtro por estado ENUM o por deleted
      const isEstadoFilter = [
        "ACTIVO",
        "PENDIENTE",
        "BLOQUEADO",
        "INACTIVO",
      ].includes(nextDeleted);
  ...
  const usersForTable = useMemo(() => {
    return (users || []).map((u) => {
      ...
      const roleSlugs = Array.isArray(roles)
        ? roles.map((r) => r?.slug).filter(Boolean)
        : [];
      const roleIds = Array.isArray(roles)
        ? roles.map((r) => r?.id).filter(Boolean)
        : [];
      return {
        id: u?.id,
        username: u?.username,
  ...
  /**
   * onSubmit
   * Maneja la creación del usuario: transforma campos, valida rol y envía payload al backend.
   * - Capitaliza nombres y aplica convenciones (apellidos en mayúsculas).
   *
  ...
  const onSubmit = async (form) => {
    try {
      ...
      const personalId =
        personalIdRaw && personalIdRaw !== "" ? Number(personalIdRaw) : null;
      const estadoInicial =
        personalId && !Number.isNaN(personalId) ? "ACTIVO" : "PENDIENTE";
  ...
  /**
   * handleApplyFilters
   * Aplica filtros y recarga la lista en la primera página.
   */
  ...
  /**
   * handlePrev
   * Navega a la página anterior y recarga la lista.
   */
  ...
  /**
   * handleNext
   * Navega a la página siguiente y recarga la lista.
   */
  ...
  /**
   * handleToggleEstado
   * Alterna el estado de un usuario entre ACTIVO/INACTIVO.
   *
  ...
  const handleToggleEstado = async (u) => {
    try {
      ...
      toast.success(
        `Usuario ${nextEstado === "ACTIVO" ? "activado" : "inactivado"}`
      );
  ...
  const handleRestore = async (u) => {
    try {
      ...
      toast.error(
        err?.response?.data?.message || err?.message || "No se pudo reactivar"
      );
    }
  };
        ...
        <div className="mt-6">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-300 mt-1">
                Busca y valida antes de crear. Puedes activar/inactivar y (si
                eres super admin) eliminar.
              </p>
            </div>
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-sm text-slate-500 dark:text-slate-300">
              Página {usersPagination?.page ?? page} de{" "}
              {usersPagination?.totalPages ?? 1} ({usersPagination?.total ?? 0}{" "}
              registros)
            </p>
            <div className="flex gap-2">
              <button
        ...
        <div className="mt-6 relative">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-300 mt-1">
                Completa el formulario y asigna un rol.
              </p>
            </div>
            <button
              type="button"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  } bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50 placeholder:text-slate-400 dark:placeholder:text-slate-500/60 focus:outline-none focus:ring-2 focus:ring-primary-600/25`}
            ...
            <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4">
              ...
              >
                <option value="">
                  {loadingPersonal
                    ? "Cargando…"
                    : "— Sin asignar (estado PENDIENTE) —"}
                </option>
                {personalList.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.apellido_paterno} {p.apellido_materno}, {p.nombres} -{" "}
                    {p.doc_numero || "Sin Doc"}
                  </option>
                ))}
              </select>
              <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">
                <strong>Importante:</strong> Si asigna un personal, el usuario
                se creará como <strong>ACTIVO</strong>. Si no asigna, se creará
                como <strong>PENDIENTE</strong> y no podrá iniciar sesión hasta
                que se le asigne.
              </p>
            </div>
            ...
            <div className="mt-4 space-y-4 max-h-[60vh] overflow-y-auto pr-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditDraft((d) => ({
                        ...d,
                        nombres: capitalize(e.target.value),
                      }))
                    }
                    className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50 placeholder:text-slate-400 dark:placeholder:text-slate-500/60 focus:outline-none focus:ring-2 focus:ring-primary-600/25"
                  />
              ...
              <div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const newEstado =
                      newPersonalId && editDraft.estado === "PENDIENTE"
                        ? "ACTIVO"
                        : editDraft.estado;
                    setEditDraft((d) => ({
                      ...d,
                      personal_seguridad_id: newPersonalId,
                      estado: newEstado,
                    }));
                  }}
                  className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50 focus:outline-none focus:ring-2 focus:ring-primary-600/25"
                ...
                >
                  <option value="">
                    {loadingPersonal ? "Cargando…" : "— Sin asignar —"}
                  </option>
                  {personalList
                    .filter((p) => {
                      ...
                      if (
                        editDraft.personal_seguridad_id &&
                        String(p.id) === String(editDraft.personal_seguridad_id)
                      )
                        return true;
                      // Filtrar por apellidos del usuario para sugerencias más relevantes
                      if (!editingUser?.apellidos) return true;
                      const userApellidos = (editingUser.apellidos || "")
                        .toUpperCase()
                        .split(" ");
                      const personalApellidos = `${p.apellido_paterno || ""} ${
                        p.apellido_materno || ""
                      }`.toUpperCase();
                      return userApellidos.some(
                        (ap) => ap && personalApellidos.includes(ap)
                      );
                    })
                    .map((p) => (
                      <option key={p.id} value={String(p.id)}>
                        {p.apellido_paterno} {p.apellido_materno}, {p.nombres} -{" "}
                        {p.doc_numero || "Sin DNI"}
                      </option>
                    ))}
                </select>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditDraft((d) => ({ ...d, estado: e.target.value }))
                  }
                  className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50 focus:outline-none focus:ring-2 focus:ring-primary-600/25"
                >
