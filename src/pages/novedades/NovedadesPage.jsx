...
import { 
  AlertTriangle, ChevronLeft, ChevronRight, Eye, Plus, RefreshCw, 
  Search, Trash2, X, MapPin, Phone, User, FileText, Clock, Calendar,
  Bell, Radio, Camera, Car, Users, Shield
} from 'lucide-react'

import { 
  listNovedades, deleteNovedad, createNovedad, 
  listTiposNovedad, listEstadosNovedad, listSubtiposNovedad,
  listSectores, listCuadrantes, listUbigeos, asignarRecursos,
  listUnidadesOficina, listVehiculos, listPersonalSeguridad,
  getHistorialEstados, getNovedadById
...
export default function NovedadesPage() {
  ...
  useEffect(() => {
    ...
    if (novedadId && novedades.length > 0) {
      ...
      if (novedad) {
        ...
        getNovedadById(novedadId).then(data => {
          if (data) {
  ...
  const resetForm = () => {
    setFormData({
      tipo_novedad_id: '',
      subtipo_novedad_id: '',
      fecha_hora_ocurrencia: new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16),
      origen_llamada: 'TELEFONO_107',
      localizacion: '',
      referencia_ubicacion: '',
      sector_id: '',
  ...
  const handleCreate = async () => {
    if (!formData.tipo_novedad_id || !formData.subtipo_novedad_id || !formData.descripcion) {
      toast.error('Complete los campos requeridos: Tipo, Subtipo y Descripción')
      return
    }
    ...
    try {
      await createNovedad({
        ...
        cuadrante_id: formData.cuadrante_id ? Number(formData.cuadrante_id) : undefined,
        ...
        reportante_doc_identidad: formData.reportante_doc_identidad 
          ...
          : undefined,
        num_personas_afectadas: formData.num_personas_afectadas ? Number(formData.num_personas_afectadas) : undefined,
  ...
  const openAtencionModal = async (novedad) => {
    ...
    setAtencionData({
      ...
      fecha_despacho: novedad.fecha_despacho ? new Date(novedad.fecha_despacho).toISOString().slice(0, 16) : new Date(Date.now() - new Date().getTimezoneOffset() * 60000).toISOString().slice(0, 16),
      turno: novedad.turno || '',
      ...
      fecha_llegada: novedad.fecha_llegada ? new Date(novedad.fecha_llegada).toISOString().slice(0, 16) : '',
      fecha_cierre: novedad.fecha_cierre ? new Date(novedad.fecha_cierre).toISOString().slice(0, 16) : '',
      km_inicial: novedad.km_inicial || '',
      km_final: novedad.km_final || '',
      fecha_proxima_revision: novedad.fecha_proxima_revision ? new Date(novedad.fecha_proxima_revision).toISOString().slice(0, 10) : '',
      perdidas_materiales_estimadas: novedad.perdidas_materiales_estimadas || '',
    })
    setAtencionTab(0)
    setHistorialEstados([])
    setShowAtencionModal(true)
    // Cargar catálogos si no están cargados
    if (unidadesOficina.length === 0 || vehiculos.length === 0 || personalSeguridad.length === 0) {
  ...
  const handleGuardarAtencion = async () => {
    ...
    if (personalIds.length !== uniqueIds.size) {
      toast.error('No puede asignar la misma persona en múltiples campos de personal')
      return
    }
    ...
    try {
      await asignarRecursos(selectedNovedad.id, {
        unidad_oficina_id: atencionData.unidad_oficina_id ? Number(atencionData.unidad_oficina_id) : undefined,
        vehiculo_id: atencionData.vehiculo_id ? Number(atencionData.vehiculo_id) : undefined,
        personal_cargo_id: atencionData.personal_cargo_id ? Number(atencionData.personal_cargo_id) : undefined,
        personal_seguridad2_id: atencionData.personal_seguridad2_id ? Number(atencionData.personal_seguridad2_id) : undefined,
        personal_seguridad3_id: atencionData.personal_seguridad3_id ? Number(atencionData.personal_seguridad3_id) : undefined,
        personal_seguridad4_id: atencionData.personal_seguridad4_id ? Number(atencionData.personal_seguridad4_id) : undefined,
        fecha_despacho: atencionData.fecha_despacho || undefined,
        turno: atencionData.turno || undefined,
        tiempo_respuesta_minutos: atencionData.tiempo_respuesta_minutos ? Number(atencionData.tiempo_respuesta_minutos) : undefined,
        observaciones: atencionData.observaciones || undefined,
        estado_novedad_id: atencionData.estado_novedad_id ? Number(atencionData.estado_novedad_id) : undefined,
        ...
        km_inicial: atencionData.km_inicial ? Number(atencionData.km_inicial) : undefined,
        km_final: atencionData.km_final ? Number(atencionData.km_final) : undefined,
        fecha_proxima_revision: atencionData.fecha_proxima_revision || undefined,
        perdidas_materiales_estimadas: atencionData.perdidas_materiales_estimadas ? Number(atencionData.perdidas_materiales_estimadas) : undefined,
      })
  ...
  const prioridadColor = (prioridad) => {
    switch (prioridad) {
      ...
      case 'BAJA': return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300'
  ...
  const estadoColor = (estado) => {
    ...
    if (nombre.includes('registrad')) return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
    if (nombre.includes('atenci') || nombre.includes('proceso')) return 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300'
    if (nombre.includes('despach')) return 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300'
    if (nombre.includes('resuel') || nombre.includes('cerrad')) return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300'
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-50">Novedades</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Registro de incidentes y novedades</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Código</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Fecha/Hora</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Tipo</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200 hidden md:table-cell">Ubicación</th>
                <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Prioridad</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-4 py-8 text-center text-slate-500">No hay registros</td></tr>
              ) : (
                novedades.map((n) => (
                  <tr key={n.id} className={`hover:bg-slate-50 dark:hover:bg-slate-800/50 ${n.deleted_at ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-slate-700 dark:text-slate-200">
                      <div className="flex flex-col">
                        <span className="font-medium">{n.novedadTipoNovedad?.nombre || '—'}</span>
                        <span className="text-xs text-slate-500">{n.novedadSubtipoNovedad?.nombre || ''}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${prioridadColor(n.prioridad_actual)}`}>
                        {n.prioridad_actual || 'MEDIA'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${typeof estadoColor(n.novedadEstado) === 'string' ? estadoColor(n.novedadEstado) : ''}`} style={typeof estadoColor(n.novedadEstado) === 'object' ? estadoColor(n.novedadEstado) : {}}>
          ...
          <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 shadow-xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-3">
                <svg className="w-8 h-8" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <linearGradient id="shieldGradCreate" x1="0%" y1="0%" x2="0%" y2="100%">
                  ...
                  <path d="M16 2 L28 6 L28 14 C28 22 22 28 16 30 C10 28 4 22 4 14 L4 6 Z" 
                        fill="url(#shieldGradCreate)" stroke="#1a2e14" strokeWidth="1"/>
                  <text x="16" y="20" fontFamily="Arial, sans-serif" fontSize="14" fontWeight="bold" 
                        fill="#FFFFFF" textAnchor="middle">C</text>
                </svg>
                <div>
                  <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50">Nueva Novedad</h3>
                  <p className="text-sm text-slate-500 dark:text-slate-400">Complete los datos del incidente</p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {activeTab === 0 && (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Tipo de Novedad *</label>
                      <select
                        id="tipo_novedad_select"
                        value={formData.tipo_novedad_id}
                        onChange={(e) => {
                          setFormData({ ...formData, tipo_novedad_id: e.target.value, subtipo_novedad_id: '' })
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Subtipo *</label>
                      <select
                        value={formData.subtipo_novedad_id}
                        onChange={(e) => setFormData({ ...formData, subtipo_novedad_id: e.target.value })}
                      ...
                      >
                        ...
                        {subtipos.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}
                      </select>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Fecha y Hora de Ocurrencia *</label>
                    <input
                      type="datetime-local"
                      value={formData.fecha_hora_ocurrencia}
                      onChange={(e) => setFormData({ ...formData, fecha_hora_ocurrencia: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200 mb-2">Origen de la Llamada</label>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      {ORIGEN_LLAMADA_OPTIONS.map((opt) => {
                        const Icon = opt.icon
                        const isSelected = formData.origen_llamada === opt.value
                        return (
                          <button
                            key={opt.value}
                            type="button"
                            onClick={() => setFormData({ ...formData, origen_llamada: opt.value })}
              ...
              {activeTab === 1 && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Dirección / Localización</label>
                    <input
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Referencia</label>
                    <input
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Sector *</label>
                      <select
                        value={formData.sector_id}
                        onChange={(e) => {
                          setFormData({ ...formData, sector_id: e.target.value, cuadrante_id: '' })
                          fetchCuadrantes(e.target.value)
                        }}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                        <option value="">Seleccione sector...</option>
                        {sectores.map((s) => <option key={s.id} value={s.id}>{s.sector_code} - {s.nombre}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Cuadrante *</label>
                      <select
                      ...
                      >
                        ...
                        {cuadrantes.map((c) => <option key={c.id} value={c.id}>{c.cuadrante_code} - {c.nombre}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">UBIGEO</label>
                        ...
                        {ubigeos.length > 0 && (
                          <div className="absolute z-10 w-full mt-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                            {ubigeos.map((u) => (
                              ...
                              >
                                <span className="font-semibold text-primary-600 dark:text-primary-400">{u.ubigeo_code}</span>
                                <span className="text-slate-600 dark:text-slate-300"> - {u.departamento}/{u.provincia}/{u.distrito}</span>
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      {formData.ubigeo_code && (
                        <p className="mt-1 text-xs text-green-600">Seleccionado: {formData.ubigeo_code}</p>
                      )}
                    </div>
                  </div>
                  {/* Botón GPS */}
                  <div className="flex items-center justify-between p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <label htmlFor="gps_enabled" className="text-sm font-medium text-slate-700 dark:text-slate-200">
                    ...
                    {gpsEnabled && !gpsLoading && (
                      <span className="text-xs text-green-600 dark:text-green-400 font-medium">✓ GPS activo</span>
                    )}
                  </div>

                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Latitud</label>
                      <input
                        type="number"
                        step="any"
                        ...
                        readOnly={gpsEnabled}
                        className={`mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50 ${gpsEnabled ? 'bg-slate-100 dark:bg-slate-800 cursor-not-allowed' : ''}`}
                        placeholder="-12.0464"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Longitud</label>
                      <input
                        type="number"
                        step="any"
              ...
              {activeTab === 2 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                    ...
                    />
                    <label htmlFor="es_anonimo" className="text-sm font-medium text-slate-700 dark:text-slate-200">
                      Reporte anónimo
                    </label>
                  </div>
                  {!formData.es_anonimo && (
                    <>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Nombre del Reportante</label>
                        <input
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setFormData({ ...formData, reportante_telefono: e.target.value })}
                            className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                        ...
                        <div>
                          ...
                          <select
                            ...
                            onChange={(e) => setFormData({ ...formData, reportante_tipo_doc: e.target.value })}
                            className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                          >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setFormData({ ...formData, reportante_doc_identidad: e.target.value })}
                            className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                            placeholder="Número de documento"
                          />
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Nro. Personas Afectadas</label>
                          <input
                            type="number"
                            min="0"
                            value={formData.num_personas_afectadas}
                            onChange={(e) => setFormData({ ...formData, num_personas_afectadas: e.target.value })}
                            className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                            placeholder="0"
                          />
                        </div>
              ...
              {activeTab === 3 && (
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Descripción del Incidente *</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Observaciones Adicionales</label>
                    <textarea
          ...
          <div className="w-full max-w-3xl rounded-2xl bg-white dark:bg-slate-900 shadow-xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-start justify-between p-6 border-b border-slate-200 dark:border-slate-700">
              <div className="flex items-start gap-4">
                <svg className="w-10 h-10" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                  <defs>
                    <linearGradient id="shieldGradView" x1="0%" y1="0%" x2="0%" y2="100%">
                      <stop offset="0%" style={{stopColor:'#4F7942',stopOpacity:1}} />
                      <stop offset="100%" style={{stopColor:'#2D4A22',stopOpacity:1}} />
                    </linearGradient>
                  </defs>
                  <path d="M16 2 L28 6 L28 14 C28 22 22 28 16 30 C10 28 4 22 4 14 L4 6 Z" 
                        fill="url(#shieldGradView)" stroke="#1a2e14" strokeWidth="1"/>
                  <text x="16" y="20" fontFamily="Arial, sans-serif" fontSize="14" fontWeight="bold" 
                        fill="#FFFFFF" textAnchor="middle">C</text>
                </svg>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-sm text-slate-500">#{viewingNovedad.novedad_code}</span>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${prioridadColor(viewingNovedad.prioridad_actual)}`}>
                      {viewingNovedad.prioridad_actual || 'MEDIA'}
                    </span>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${typeof estadoColor(viewingNovedad.novedadEstado) === 'string' ? estadoColor(viewingNovedad.novedadEstado) : ''}`} style={typeof estadoColor(viewingNovedad.novedadEstado) === 'object' ? estadoColor(viewingNovedad.novedadEstado) : {}}>
                  ...
                  <p className="text-sm text-slate-500">{viewingNovedad.novedadSubtipoNovedad?.nombre || ''}</p>
                </div>
              </div>
              <button
                onClick={() => { setViewingNovedad(null); setViewingTab(0) }}
                className="p-2 rounded-lg text-slate-500 hover:text-slate-700 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800"
              >
                <X size={20} />
              </button>
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-700">
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${viewingTab === 0 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${viewingTab === 1 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${viewingTab === 2 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${viewingTab === 3 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
              ...
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${viewingTab === 4 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
              ...
              {viewingTab === 0 && (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-slate-900 dark:text-slate-50 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">{viewingNovedad.descripcion || '—'}</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Tipo de Novedad</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50 font-medium">{viewingNovedad.novedadTipoNovedad?.nombre || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Subtipo</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadSubtipoNovedad?.nombre || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Fecha/Hora Ocurrencia</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{formatFecha(viewingNovedad.fecha_hora_ocurrencia)}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Origen de Llamada</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                        {ORIGEN_LLAMADA_OPTIONS.find(o => o.value === viewingNovedad.origen_llamada)?.label || viewingNovedad.origen_llamada || '—'}
                      </p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Prioridad</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.prioridad_actual || 'MEDIA'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Estado</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadEstado?.nombre || '—'}</p>
                    </div>
                  </div>
                  {viewingNovedad.observaciones && (
                    <div>
                      <h4 className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-2">Observaciones</h4>
                      <p className="text-sm text-slate-700 dark:text-slate-300 p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">{viewingNovedad.observaciones}</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.localizacion || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Referencia</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.referencia_ubicacion || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Sector</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadSector?.nombre || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Cuadrante</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadCuadrante?.nombre || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Ubigeo</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.ubigeo_code || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Coordenadas GPS</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                  ...
                  {viewingNovedad.es_anonimo ? (
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        ...
                        <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.reportante_nombre || '—'}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <span className="text-xs font-medium text-slate-500">Teléfono</span>
                        <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.reportante_telefono || '—'}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <span className="text-xs font-medium text-slate-500">Documento de Identidad</span>
                        <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.reportante_doc_identidad || '—'}</p>
                      </div>
                      <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                        <span className="text-xs font-medium text-slate-500">Personas Afectadas</span>
                        <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.num_personas_afectadas || '—'}</p>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadUnidadOficina?.nombre || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Vehículo</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.novedadVehiculo?.placa || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Personal a Cargo</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                        {viewingNovedad.novedadPersonalCargo ? `${viewingNovedad.novedadPersonalCargo.nombres} ${viewingNovedad.novedadPersonalCargo.apellido_paterno}` : '—'}
                      </p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Personal #2</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                        {viewingNovedad.novedadPersonal2 ? `${viewingNovedad.novedadPersonal2.nombres} ${viewingNovedad.novedadPersonal2.apellido_paterno}` : '—'}
                      </p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Personal #3</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                        {viewingNovedad.novedadPersonal3 ? `${viewingNovedad.novedadPersonal3.nombres} ${viewingNovedad.novedadPersonal3.apellido_paterno}` : '—'}
                      </p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Personal #4</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">
                        {viewingNovedad.novedadPersonal4 ? `${viewingNovedad.novedadPersonal4.nombres} ${viewingNovedad.novedadPersonal4.apellido_paterno}` : '—'}
                      </p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Fecha Despacho</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.fecha_despacho ? formatFecha(viewingNovedad.fecha_despacho) : '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Turno</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.turno || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Tiempo Respuesta</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.tiempo_respuesta_minutos ? `${viewingNovedad.tiempo_respuesta_minutos} min` : '—'}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-4">
                  <div className="p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                    <span className="text-sm font-medium text-amber-800 dark:text-amber-200">
                      Requiere Seguimiento: {viewingNovedad.requiere_seguimiento ? 'Sí' : 'No'}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Fecha Llegada</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.fecha_llegada ? formatFecha(viewingNovedad.fecha_llegada) : '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Fecha Cierre</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.fecha_cierre ? formatFecha(viewingNovedad.fecha_cierre) : '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Km Inicial</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.km_inicial || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Km Final</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.km_final || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Fecha Próxima Revisión</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.fecha_proxima_revision || '—'}</p>
                    </div>
                    <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                      <span className="text-xs font-medium text-slate-500">Pérdidas Materiales Estimadas</span>
                      <p className="text-sm text-slate-900 dark:text-slate-50">{viewingNovedad.perdidas_materiales_estimadas ? `S/. ${viewingNovedad.perdidas_materiales_estimadas}` : '—'}</p>
                    </div>
                  </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-700">
              <div className="flex items-center gap-3">
                <svg className="w-8 h-8" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
                  ...
                  <path d="M16 2 L28 6 L28 14 C28 22 22 28 16 30 C10 28 4 22 4 14 L4 6 Z" 
                        fill="url(#shieldGradAtencion)" stroke="#1a2e14" strokeWidth="1"/>
                  <text x="16" y="20" fontFamily="Arial, sans-serif" fontSize="14" fontWeight="bold" 
                        fill="#FFFFFF" textAnchor="middle">C</text>
                </svg>
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    {selectedNovedad.novedad_code} - {selectedNovedad.novedadTipoNovedad?.nombre}
                  </p>
                </div>
              </div>
              <button
                onClick={() => { setShowAtencionModal(false); setSelectedNovedad(null) }}
            ...
            <div className="flex border-b border-slate-200 dark:border-slate-700">
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${atencionTab === 0 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Users size={16} className="inline mr-2" />
                Recursos Asignados
              </button>
              <button
                ...
                className={`flex-1 px-4 py-3 text-sm font-medium ${atencionTab === 1 ? 'text-primary-600 border-b-2 border-primary-600' : 'text-slate-500 hover:text-slate-700'}`}
              >
                <Clock size={16} className="inline mr-2" />
                Seguimiento
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-6">
              ...
              {atencionTab === 0 && (
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Unidad/Oficina</label>
                      <select
                        value={atencionData.unidad_oficina_id}
                        onChange={(e) => setAtencionData({ ...atencionData, unidad_oficina_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setAtencionData({ ...atencionData, vehiculo_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Personal a Cargo (Principal)</label>
                      <select
                        value={atencionData.personal_cargo_id}
                        onChange={(e) => setAtencionData({ ...atencionData, personal_cargo_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                        <option value="">Seleccione personal...</option>
                        {personalSeguridad.map((p) => (
                          <option key={p.id} value={p.id}>{p.doc_identidad || p.codigo} - {p.nombres} {p.apellido_paterno}</option>
                        ))}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Personal Seguridad #2</label>
                      <select
                        value={atencionData.personal_seguridad2_id}
                        onChange={(e) => setAtencionData({ ...atencionData, personal_seguridad2_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                        <option value="">Seleccione personal...</option>
                        {personalSeguridad.map((p) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Personal Seguridad #3</label>
                      <select
                        value={atencionData.personal_seguridad3_id}
                        onChange={(e) => setAtencionData({ ...atencionData, personal_seguridad3_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                        <option value="">Seleccione personal...</option>
                        {personalSeguridad.map((p) => (
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Personal Seguridad #4</label>
                      <select
                        value={atencionData.personal_seguridad4_id}
                        onChange={(e) => setAtencionData({ ...atencionData, personal_seguridad4_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                        <option value="">Seleccione personal...</option>
                        {personalSeguridad.map((p) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Estado de Novedad *</label>
                      <select
                        value={atencionData.estado_novedad_id}
                        onChange={(e) => setAtencionData({ ...atencionData, estado_novedad_id: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Fecha/Hora Despacho</label>
                      <input
                        type="datetime-local"
                        value={atencionData.fecha_despacho}
                        onChange={(e) => setAtencionData({ ...atencionData, fecha_despacho: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Turno</label>
                      <select
                        value={atencionData.turno}
                        onChange={(e) => setAtencionData({ ...atencionData, turno: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                      >
                    ...
                    <div>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Tiempo Respuesta (min)</label>
                      <input
                        type="number"
                        value={atencionData.tiempo_respuesta_minutos}
                        onChange={(e) => setAtencionData({ ...atencionData, tiempo_respuesta_minutos: e.target.value })}
                        className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                        placeholder="Minutos"
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setAtencionData({ ...atencionData, observaciones: e.target.value })}
                      className="mt-1 w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50"
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                    <input
                      ...
                      onChange={(e) => setAtencionData({ ...atencionData, requiere_seguimiento: e.target.checked })}
                      className="w-5 h-5 rounded border-slate-300 text-primary-600 focus:ring-primary-500"
                    />
                    <label htmlFor="requiere_seguimiento" className="text-sm font-medium text-amber-800 dark:text-amber-200">
                      ¿Requiere Seguimiento? (Habilita campos adicionales en pestaña Seguimiento)
                    </label>
                  </div>
                </div>
              )}
              ...
              {atencionTab === 1 && (
                <div className="space-y-4">
                  <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-800/50">
                    <h4 className="font-medium text-slate-900 dark:text-slate-50 mb-3">Información de la Novedad</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        <span className="text-slate-500">Fecha ocurrencia:</span>
                        <span className="ml-2 font-medium text-slate-900 dark:text-slate-50">
                  ...
                  <div className="p-4 rounded-lg border border-slate-200 dark:border-slate-700">
                    <h4 className="font-medium text-slate-900 dark:text-slate-50 mb-3">Recursos Asignados Actuales</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="text-slate-500">Unidad:</span>
                        <span className="ml-2 font-medium text-slate-900 dark:text-slate-50">
                          {selectedNovedad.novedadUnidadOficina?.nombre || selectedNovedad.unidad_oficina_id || '—'}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500">Vehículo:</span>
                        <span className="ml-2 font-medium text-slate-900 dark:text-slate-50">
                          {selectedNovedad.novedadVehiculo?.placa || selectedNovedad.vehiculo_id || '—'}
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500">Fecha despacho:</span>
                        <span className="ml-2 font-medium text-slate-900 dark:text-slate-50">
                          {selectedNovedad.fecha_despacho ? formatFecha(selectedNovedad.fecha_despacho) : '—'}
                        </span>
                      </div>
                      <div>
                  ...
                  <div className={`p-4 rounded-lg border ${atencionData.requiere_seguimiento ? 'border-primary-300 dark:border-primary-700 bg-primary-50/50 dark:bg-primary-900/10' : 'border-slate-200 dark:border-slate-700 bg-slate-100/50 dark:bg-slate-800/30'}`}>
                    <h4 className="font-medium text-slate-900 dark:text-slate-50 mb-3">
                      Datos de Seguimiento 
                      {!atencionData.requiere_seguimiento && <span className="text-xs text-slate-500 ml-2">(Solo lectura - Active "Requiere Seguimiento" para editar)</span>}
                    </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          className={`mt-1 w-full rounded-lg border px-3 py-2 ${atencionData.requiere_seguimiento ? 'border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-950/40 text-slate-900 dark:text-slate-50' : 'border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-500 cursor-not-allowed'}`}
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Fecha Cierre</label>
                        <input
                          type="datetime-local"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Km Inicial</label>
                        <input
                          type="number"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Km Final</label>
                        <input
                          type="number"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Fecha Próxima Revisión</label>
                        <input
                          type="date"
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-200">Pérdidas Materiales Estimadas (S/.)</label>
                        <input
                          type="number"
                          step="0.01"
