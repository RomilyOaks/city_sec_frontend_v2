/**
 * File: c:\\Project\\city_sec_frontend_v2\\src\\pages\\personal\\PersonalPage.jsx
 * @version 2.0.0
 * @description Página para la gestión del personal de seguridad: listado, creación, edición, ver detalle.
 * Incluye validaciones de documento, búsqueda de ubigeos y asignación de vehículo/cargo.
 *
 * Documentación educativa: se añadieron JSDoc y comentarios explicativos en funciones clave sin tocar la lógica.
 *
 * Organización: constantes y validadores arriba, estado y hooks, funciones de fetch/CRUD y helpers, render al final.
 *
 * @module src/pages/personal/PersonalPage.jsx
 */
...
const DOC_VALIDATION_RULES = {
  ...
  "Carnet Extranjeria": {
    pattern: /^[A-Z0-9]{9}$/i,
    msg: "El Carnet de Extranjería debe tener 9 caracteres alfanuméricos",
  },
  Pasaporte: {
    pattern: /^[A-Z0-9]{6,12}$/i,
    msg: "El Pasaporte debe tener entre 6 y 12 caracteres alfanuméricos",
  },
  PTP: {
    pattern: /^[A-Z0-9]{6,15}$/i,
    msg: "El PTP debe tener entre 6 y 15 caracteres",
  },
};
 ...
 * - Maneja arrays de errores y campos `message` o `error`.
 *
 * @param {Error} err
 * @returns {string}
 */
...
/**
 * PersonalPage - Gestión del personal de seguridad
 *
 * @version 2.0.0
 * @component
 * @category Pages
 * @description Lista, crea y edita personal de seguridad. Incluye validaciones de documento y búsqueda de ubigeos.
 *
 * Reglas:
 *  - Sólo se añade documentación y explicaciones (no se modifica la lógica existente).
 *
 * @returns {JSX.Element}
 */
...
export default function PersonalPage() {
  ...
  /**
   * fetchPersonal
   * - Consulta los datos de personal con paginación y filtros (status, búsqueda).
   * - Maneja permisos y actualiza `personal` y `pagination`.
   *
   * @param {Object} [options]
   * @param {number} [options.nextPage=1]
   * @param {string} [options.searchOverride]
   * @param {string} [options.statusOverride]
  ...
  /**
   * validateDocumento
   * Valida el número de documento según el tipo seleccionado (DNI, Carnet Extranjería, etc.).
   *
   ...
   * - Consulta el endpoint para verificar si ya existe un personal con el mismo documento.
   * - Se ejecuta al salir del campo para evitar duplicados.
   *
   * @returns {Promise<void>}
   */
  ...
  const checkDocumentoDuplicado = async () => {
    ...
    try {
      const result = await listPersonal({
        search: formData.doc_numero,
        limit: 10,
      });
      const list = result?.personal || result || [];
      const existe = list.find(
        (p) =>
          p.doc_numero === formData.doc_numero &&
          p.doc_tipo === formData.doc_tipo
      );
      if (existe) {
        setDocDuplicadoError(
          `Ya existe un personal con ${formData.doc_tipo}: ${formData.doc_numero}`
        );
      } else {
        setDocDuplicadoError("");
      }
  ...
  /**
   * handleCreate
   * Valida los campos obligatorios y envía la creación del personal al backend.
   * - Convierte `cargo_id`/`vehiculo_id` a números y limpia campos vacíos.
   * - Muestra mensajes de resultado y refresca la lista.
   *
   * @returns {Promise<void>}
   */
  const handleCreate = async () => {
    if (
      ...
      toast.error(
        "Complete los campos requeridos (documento, nombres y apellidos)"
      );
      return;
    }
  ...
  /**
   * handleUpdate
   * Envía la actualización del registro de personal al backend. Omite documento en la actualización.
   *
   * @returns {Promise<void>}
   */
  ...
  /**
   * openEditModal
   * Abre el modal de edición y precarga el formulario con los datos del personal seleccionado.
   *
   * @param {Object} p - registro de personal
   */
  ...
  /**
   * statusColor
   * Retorna clases CSS para el estado del trabajador (Activo, Inactivo, etc.).
   *
   * @param {string} status
   * @returns {string}
   */
  ...
  /**
   * isBasicosComplete
   * Indica si la pestaña de datos básicos contiene los campos obligatorios.
   *
   * @returns {boolean}
   */
  ...
  /**
   * isLicenciaComplete
   * Indica si la pestaña de licencia/vehículo tiene datos para considerarla completa.
   *
   * @returns {boolean}
   */
    ...
    const tabs = [
      {
        ...
        label: "Datos Básicos",
        num: 1,
        isComplete: isBasicosComplete,
      },
      {
        id: "adicionales",
        label: "Licencia / Vehículo",
        num: 2,
        isComplete: isLicenciaComplete,
      },
    ];
          ...
          <div className="grid grid-cols-2 gap-4">
              ...
              <>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, doc_tipo: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    } bg-white dark:bg-slate-950/40 px-3 py-2 text-slate-900 dark:text-slate-50`}
                  />
                  {docDuplicadoError && (
                    <p className="mt-1 text-xs text-red-600 flex items-center gap-1">
                      <span className="inline-block w-4 h-4 bg-red-500 text-white rounded-full text-center leading-4 text-xs">
                        ✕
                      </span>
                      {docDuplicadoError}
                    </p>
                ...
                <div className="mt-1 max-h-40 overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-900 shadow-lg z-10 relative">
                  {ubigeos.map((u) => (
                    ...
                    >
                      ...
                      <span className="text-emerald-600 dark:text-emerald-400 text-sm font-bold float-right">
                        {u.ubigeo_code}
                      </span>
                    </button>
                  ))}
            ...
            <div className="col-span-2 mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-sm text-blue-700 dark:text-blue-300 flex items-start gap-2">
                ...
                <span>
                  <strong>Tip:</strong> Puede agregar información de licencia y
                  vehículo usando la pestaña
                  <button
                    type="button"
          ...
          <div className="space-y-4">
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
              <p className="text-sm text-blue-700 dark:text-blue-300">
                <strong>Nota:</strong> Los datos de licencia y vehículo solo
                aplican para personal que conduce vehículos (conductores).
                Policías, serenos y otro personal que participa en operativos
                pero no conduce, no requieren estos datos.
              </p>
            </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      licencia: e.target.value.toUpperCase(),
                    })
                  }
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              >
                ...
                <path
                  ...
                  fill="url(#shieldGradPersonalCreate)"
                  stroke="#1a2e14"
                  strokeWidth="1"
                />
                <text
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50 mb-4">
              Editar: {editingPersonal.nombres}{" "}
              {editingPersonal.apellido_paterno}
            </h3>
            {renderFormTabs(true)}
            <div className="mt-6 flex items-center justify-between border-t border-slate-200 dark:border-slate-700 pt-4">
              <span className="text-xs text-slate-400">
      ...
      {viewingPersonal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-slate-900 p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-50">
              {viewingPersonal.nombres} {viewingPersonal.apellido_paterno}{" "}
              {viewingPersonal.apellido_materno}
            </h3>
              ...
              <div className="col-span-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <span className="text-slate-900 dark:text-slate-50">
                      {viewingPersonal.PersonalSeguridadCargo?.nombre || "—"}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-slate-900 dark:text-slate-50">
                      {viewingPersonal.regimen || "—"}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-500">F. Ingreso:</span>{" "}
                    <span className="text-slate-900 dark:text-slate-50">
                      {viewingPersonal.fecha_ingreso || "—"}
                    </span>
                  </div>
                </div>
              </div>
                    ...
                    <div>
                      ...
                      <span className="text-slate-900 dark:text-slate-50">
                        {viewingPersonal.PersonalSeguridadVehiculo
                          ? `${
                              viewingPersonal.PersonalSeguridadVehiculo.placa
                            } (${
                              viewingPersonal.PersonalSeguridadVehiculo
                                .codigo_vehiculo || "Sin código"
                            })`
                          : "—"}
                      </span>
                    </div>
